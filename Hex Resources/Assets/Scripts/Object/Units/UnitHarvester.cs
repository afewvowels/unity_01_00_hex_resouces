...
public class UnitHarvester : HexUnit
{
	...
	void FixedUpdate()
	{
		...
		{
			Debug.Log("error doing harvester actions");
		}
	}
	...
	private IEnumerator DoAdjacent()
	{
		...
		else if (destination.Building)
		{
			...
			yield return LookAt(Location.Position);
	...
	private void SetResourcesInBucket()
	{
		if (HeldResources == maxHeldResources)
		{
			...
			{
				Debug.Log("error setting max held resources on harvester");
			}
		}
		else if (HeldResources > maxHeldResources * 0.66f)
		{
			...
			{
				Debug.Log("error setting medium held resources on harvester");
			}
		}
		else if (HeldResources > maxHeldResources * 0.33f)
		{
			...
			{
				Debug.Log("error setting low held resources on harvester");
			}
		}
		else
		{
			...
			{
				Debug.Log("error setting no held resources on harvester");
			}
		}
	}
