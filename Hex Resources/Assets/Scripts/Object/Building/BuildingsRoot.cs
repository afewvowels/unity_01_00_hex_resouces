...
public class BuildingsRoot : MonoBehaviour
{
    ...
    public BuildingProcessing[] buildings;
    ...
    private void RefreshBuildingsList()
    {
        ...
        GameObject[] buildingsBase = GameObject.FindGameObjectsWithTag("processing");
        buildings = new BuildingProcessing[buildingsBase.Length];

        for (int i = 0; i < buildingsBase.Length; i++)
        {
            //buildings[i] = buildingsBase[i].GetComponent<BuildingBaseClass>();
            buildings[i] = buildingsBase[i].GetComponent<BuildingProcessing>();
        }
    }
    ...
    public List<HexCell> GetNearestBuildingPath(HexCell origin)
    {
        ...
        foreach (BuildingProcessing building in buildings)
        {
            grid.FindPath(origin, building.dropoffCell, 10, false);
